...
import micropython, machine, math
import time
...
class Rotary(object):
    ...
    def calc_velocity(self):
        #period = self.timer.counter()   # get timer count (10kHz so 1 period is 100 u secs)
        delta = time.ticks_diff(time.ticks_us(), self._start) # compute time difference
        velocity_old = self._velocity
        velocity = (2*math.pi*1000000/(75*4*self.CPR*delta) + velocity_old)/2
        #print(velocity)
        #velocity = (velocity + velocity_old)/2
        self._velocity = velocity
        self._start = time.ticks_us()          # reset counter
