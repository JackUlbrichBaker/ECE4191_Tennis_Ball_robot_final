...
class DiffDriveRobot:
    ...
    def __init__(self,inertia=5, dt=0.1, drag=0.2, wheel_radius=2.91, wheel_sep=28):
        ...
        self.m1 = Pin(1, Pin.OUT)
        self.m2 = Pin(2, Pin.OUT)
        self.m3 = Pin(3, Pin.OUT)
        self.m4 = Pin(4, Pin.OUT)
        
        self.enable1 = PWM(Pin(0), freq=20000, duty_u16=0)
        self.enable2 = PWM(Pin(5), freq=20000, duty_u16=0)
    ...
    def vel_control_set(self, vel, motor, direction):
        if motor == 0:
             if direction == 0:
    ...
    def pos_control_set(self, pulses, motor, direction):
        if motor == 0:
             if direction == 0:
    ...
    def pose_update(self):
        ...
        self.x = self.x + d_disp*math.cos(self.th)             # updates x position in world frame 
        self.y = self.y + d_disp*math.sin(self.th)             # updates y position in world frame 
    ...
    def vel_motor_control_loop(self):
       ...
       #     self.integrated_err_right = 10000          # set upper limit to avoid integrator windup
        ...
        duty_cycle_left = self.err_left * self.Kp + self.integrated_err_left * self.Ki
        duty_cycle_right = self.err_right * self.Kp + self.integrated_err_right * self.Ki
