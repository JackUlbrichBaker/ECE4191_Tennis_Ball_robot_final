...
while sum(circle_det_log) < circle_detect_wait_time:
    ...
    # Blur image to get rid of noise and lines on tennis ball
    ...
    # Getting contours of mask (initialize centre to largest contour)
    all_contours = cv2.findContours(mask.copy(),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    for c in all_contours:
        ...
        circ_meas = (4*np.pi*area)/(perim*perim)
        if circ_meas > circ_thres: circle_contours.append(c)

    # Create image out of final contour
    contour_mask = np.zeros_like(mask)
    contours_img = cv2.drawContours(contour_mask, all_contours, -1, (255), thickness=cv2.FILLED)

    # Edge detection ###############################################################
    # find_edge_img = contours_img[:, :, 2]

    # # Combine the edges from each channel
    edges_combined = cv2.Canny(contours_img, 110, 200,5, L2gradient=True)

    cv2.imshow('Contour of Ball', edges_combined)
    ...
    circles = cv2.HoughCircles(
        ...
        param2=10,                        # Accumulator threshold for the circle centers at the detection stage
