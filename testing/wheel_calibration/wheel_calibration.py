...
def calibrateWheelRadius():
    ...
    scale = 0
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range): # delta_time in s, wheel_vel in ticks/s
        # first find velocity from distance travelled
        linear_vel = 1/delta_time # d/t where d is in metres
        # then find scale from wheel velocity and linear velocity
        scale += (linear_vel/wheel_vel)/num # divide by num and add to total to find average
...
def calibrateBaseline(scale):
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range): 
        # use equation L = scale/theta_dot * (omega_R - omega_L) to find the length of the wheelbase
        theta_dot = (2*np.pi)/delta_time
        baseline += ((scale/theta_dot) * (2*wheel_vel))/num
...
if __name__ == "__main__":
    ...
    baseline = calibrateBaseline(0.004525)
