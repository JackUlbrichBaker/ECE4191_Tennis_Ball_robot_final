...
class Robot:
    ...
    def derivative_drive(self, drive_meas):
        ...
        # if ang_vel is zero then use simple jacobian
        if ang_vel == 0:
            ...
            # find our R value
            R = lin_vel/ang_vel
            next_theta = th + ang_vel*dt
            # is of the form lin_vel, ang_vel, th by theta, x, y
            DFx[0, 2] = R*(-np.cos(th) + np.cos(next_theta))
            DFx[1, 2] = R*(-np.sin(th) + np.sin(next_theta))
        return DFx
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            Jac2[0, 1] = lin_vel*(((-np.sin(th)+np.sin(th2))/(ang_vel*ang_vel)) + (dt*np.cos(th2)/ang_vel))
            Jac2[1, 1] = lin_vel*(-((np.cos(th)-np.cos(th2))/(ang_vel*ang_vel)) + (dt*np.sin(th2)/ang_vel))
            Jac2[2, 1] = dt
